...
def home():
    ...
    cursor.execute('''
        SELECT v.*, last_position 
        FROM videos v 
        LEFT JOIN user_video_progress uvp 
        ON v.id = video_id AND user_id = %s
        ORDER BY last_watched_at DESC LIMIT 0
    ''', (user_id,))
        ...
        try:
            ...
            try:
                ...
                with smtplib.SMTP('smtp.gmail.com', 587) as server:
                    server.starttls()
                    server.login(EMAIL_USER, EMAIL_PASS)
                    server.sendmail(EMAIL_USER, 'email@vstand4usolutions.com', msg.as_string())
